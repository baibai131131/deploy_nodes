# RV Multi HY2：TCP REALITY + Hysteria2 独立优化版
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/baibai131131/deploy_nodes/main/RV_multi_HY2.sh)
```

脚本会同时部署 TCP REALITY 与 Hysteria2，并在完成后输出三条 Clash/Mihomo 订阅链接。安装前请确认 VPS 厂商安全组允许脚本使用的 TCP、UDP 和订阅端口。

默认 TCP REALITY 目标是 `www.cloudflare.com`。安装完成前，脚本会启动一个临时本机客户端，实际完成 REALITY 握手并访问 HTTPS；只有测试成功才会显示部署完成。这比只检查 TCP/443 是否打开更能发现 SNI、证书或密钥不匹配问题。

已有安装若仍保存了旧目标，可原地切换并复用原有凭据：

```bash
REALITY_SNI=www.cloudflare.com bash <(curl -fsSL https://raw.githubusercontent.com/baibai131131/deploy_nodes/main/RV_multi_HY2.sh)
```
...
本脚本使用两个职责清晰的官方核心：Xray 只承载 TCP REALITY，官方 Hysteria 核心承载 HY2。默认将 UDP 收发缓冲安全地提高到 Hysteria 官方建议的 16 MiB，并在当前内核原生支持时启用 TCP BBR + fq；不更换内核、不安装 BBRplus、魔改模块或锐速。HY2 使用自身的用户态 BBR，与系统 TCP BBR 相互独立。卸载时会尝试恢复脚本接管前的参数。

功能包括：

- XTLS 官方安装器获取并校验 Xray-core；Hysteria 固定兼容版本并使用官方 `hashes.txt` 校验 SHA256；
- 安装前配置校验，失败不覆盖现有配置；
- 安装完成前实际测试 TCP REALITY 握手和 HTTPS 出站；
...
- TCP 与 UDP 默认都使用数字端口 `443`，协议不同不会冲突。
- HY2 默认保持标准 HTTP/3 外观并提供伪装页面；若网络专门识别或阻断 QUIC/HTTP3，可改用 `HY2_OBFS_MODE=salamander`。Salamander 会变成随机化 UDP，不再是标准 HTTP/3，两者不能同时兼得。
- 生成三份 Clash/Mihomo 订阅：仅 TCP、仅 HY2、自动测速并故障切换。
- 为兼容 Mihomo REALITY，Xray 默认固定为 `v26.6.27`，不会盲目升级到 Mihomo 文档明确警告的不兼容版本线。
- Clash 配置的本机 `mixed-port` 默认 `7897`，可用 `CLASH_MIXED_PORT` 修改；它不是 VPS 防火墙端口。
...
```bash
chmod +x RV_multi_HY2.sh
sudo ./RV_multi_HY2.sh install
```

指定端口与公网地址：

```bash
sudo REALITY_PORT=443 HY2_PORT=443 SERVER_ADDR=203.0.113.10 ./RV_multi_HY2.sh install
```
...
```bash
sudo ./RV_multi_HY2.sh show
sudo ./RV_multi_HY2.sh diagnose
sudo ./RV_multi_HY2.sh update
sudo ./RV_multi_HY2.sh restart
sudo ./RV_multi_HY2.sh uninstall
```

安装后还需要在 VPS 厂商的安全组中放行：

- `TCP/443`（或你设置的 `REALITY_PORT`）
- `UDP/443`（或你设置的 `HY2_PORT`）
...
| `HY2_PORT` | `443` | HY2 UDP 入口端口；可与 REALITY 的 TCP/443 共用数字端口 |
| `REALITY_SNI` | `www.cloudflare.com` | VPS 本机可直连的 TLS 1.3 伪装目标 |
...
| `HY2_OBFS_MODE` | `none` | `none` 保持标准 HTTP/3 伪装；`salamander` 适合 QUIC 被针对性识别但 UDP 仍可达的网络 |
| `HY2_MASQUERADE_URL` | `https://www.microsoft.com/` | HY2 HTTP/3 伪装反向代理目标 |
| `HYSTERIA_VERSION` | `2.12.3` | 固定的 Hysteria 官方版本；下载后核对官方 SHA256 |
| `ENABLE_BBR` | `1` | 启用 16 MiB UDP 缓冲，并在内核支持时启用原生 TCP BBR + fq；不会更换内核 |
| `SUB_PORT` | `18080` | Clash 订阅服务 TCP 端口 |
| `CLASH_MIXED_PORT` | `7897` | Clash 客户端本机的 mixed-port，不需要在 VPS 放行 |
...
6. 不要运行 `Linux-NetSpeed/tcp.sh` 一类旧脚本。该项目已封存，包含 Linux 4.11、第三方内核模块与跳过 TLS 校验的下载流程；现代 Ubuntu/Debian 直接使用内核自带 BBR 即可。
...
- `/etc/rv-multi-hy2/` 包含节点密钥，权限为 root-only，请不要公开。
- `uninstall` 只删除本脚本的服务、配置和对应防火墙规则，不删除可能被其他服务复用的 Xray 二进制。
- 如果安装失败，运行 `sudo ./RV_multi_HY2.sh diagnose` 查看配置校验、监听端口和最近日志。
