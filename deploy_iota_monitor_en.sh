...
# IOTA Guardian V6.3: precise status detection, registration diagnostics, Run lookup, and a local live dashboard.
[[ "$(uname -s)" == Darwin ]] || { echo "This installer supports macOS only."; exit 1; }
[[ "$(id -u)" != 0 ]] || { echo "Run this installer as the currently logged-in user. Do not use sudo."; exit 1; }
GUI_DOMAIN="gui/$(id -u)"
launchctl print "$GUI_DOMAIN" >/dev/null 2>&1 || { echo "Run this installer in Terminal from a logged-in Mac desktop session."; exit 1; }
...
trap 'echo "Installation did not complete. Backup: $BACKUP. Keep this directory private because it contains the Push URL." >&2' ZERR

echo "=============================================="
echo " IOTA Guardian V6.3 Local Dashboard"
echo "=============================================="
echo "Monitors, records, alerts, and cleans old logs only. It does not control or restart IOTA."
echo

# Reuse the previous name and Push URL automatically; prompt on a new Mac.
...
if [[ -s "$BASE/node_name" && -s "$BASE/push_url" ]]; then
  echo "Existing configuration detected: $(cat "$BASE/node_name")"
  read "KEEP?Keep the existing name and Push URL? Enter y to continue: "
else
  KEEP="n"
fi
if [[ "${KEEP:-n}" != "y" && "${KEEP:-n}" != "Y" ]]; then
  read "NODE_NAME?Enter a name for this Mac, for example B1-M01: "
  read "PUSH_URL?Paste the unique Uptime Kuma Push URL for this Mac: "
  [[ -n "$NODE_NAME" ]] || { echo "The machine name cannot be empty."; exit 1; }
  [[ "$PUSH_URL" == http://* || "$PUSH_URL" == https://* ]] || { echo "Invalid Push URL format."; exit 1; }
...
now = int(time.time())
try:
    # CLI logs can be large. Read at most the final 16 MiB to keep memory usage low.
    ...
    print("UNKNOWN\t-\t-\t-\t-\t-\t999999\tUnable to read the log\t0\t0\t-\t999999\t999999")
    raise SystemExit
...
for i, line in enumerate(lines):
    ...
    strong = None
    if re.search(r"(?:End (?:forward|backward)|(?:FORWARD|BACKWARD) complete)", line, re.I): strong = "Computation completed"
    elif re.search(r"End (upload|submit) activation", line, re.I): strong = "Activation completed"
    elif "activation push send" in low and "ack ok" in low: strong = "Activation delivered"
    elif "activation push recv" in low and ("materialize done" in low or "pulled ingress" in low): strong = "Activation received"
    elif "/miner/submit_activation; response:" in low and "error" not in low: strong = "Activation submitted"
    elif "weights submitted successfully" in low: strong = "Weights submitted"
    elif "submit_weights:" in low or "getting pseudo gradients" in low or "optimization_reset:" in low: strong = "Weight training"
    if strong:
...
# Real computation within the last five minutes takes priority over an idle heartbeat,
# but not over a newer queue, reset, or merging state.
if activity_age <= 300 and last_activity_i > barrier_i:
    state = "TRAINING"
# An old successful event must not leave the machine reported as TRAINING forever.
...
detail = last_activity_name or "No recent valid training event"
...
LABELS = {
    "TRAINING": ("🟢", "Active computation"), "MERGING": ("🟡", "Merging phase"),
    "SYNCING": ("🟡", "Weight synchronization"), "RUN_IDLE": ("🟠", "Waiting inside Run"),
    "INITIALIZING": ("🟠", "Training pool initialization"), "CONFIRMED": ("🔵", "Queue confirmation received"),
    "PROCESSING": ("🟠", "Registration processing"), "REG_FAILED": ("🔴", "Registration confirmation failed"),
    "QUEUED": ("🔵", "Registration queue"), "RESETTING": ("🔴", "Registration reset"),
    "CRASHED": ("🔴", "Process crashed"), "STOPPED": ("🔴", "IOTA stopped"),
    "STALE": ("🔴", "Log is stale"), "NO_LOG": ("🔴", "No log found"),
...
def render():
    ...
    version = (BASE / "version").read_text(errors="ignore").strip() if (BASE / "version").exists() else "unknown"
    log = latest_log()
    ...
    icon, label = LABELS.get(state, ("🔴", "Status unconfirmed"))
    free = shutil.disk_usage("/").free / 1024**3
    lines = [
        "==============================================",
        f"       IOTA GUARDIAN V{version} LOCAL LIVE MONITOR",
        "==============================================",
        "",
        f"Machine:          {node}",
        f"Current time:     {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "",
        f"Status:           {icon} {state} ({label})",
        f"IOTA process:     {'Running' if running else 'Not running'} | CPU {cpu:.1f}% | Memory {mem:.1f}%",
    ]
    if pos != "-": lines.append(f"Queue position:   {pos}")
    if state == "QUEUED": lines.append("Run:              Not assigned (queued)")
    elif state in {"CONFIRMED", "PROCESSING"}: lines.append("Run:              Waiting for server assignment")
    ...
    if p2p != "-":
        ...
        try:
            ...
            if total and ok * 100 < total * 25: warning = "  ⚠️ LOW"
        except Exception: pass
        lines.append(f"P2P:              {p2p}{warning}")
    lines += [
        f"Last valid work:  {duration(activity_age)} ago" if activity_age != "-" else "Last valid work:  N/A",
        f"Evidence:         {detail if detail != '-' else 'N/A'}",
        f"Activations:      ACK success {oks if oks != '-' else '0'} | Failed {fails if fails != '-' else '0'}",
    ]
    if reg_error != "-" and int(reg_age or 999999) <= 21600:
        lines.append(f"Recent registration issue: {reg_error} ({duration(reg_age)} ago)")
    if queue_warn_age != "-" and int(queue_warn_age or 999999) <= 600:
        lines.append("Scheduler API:    ⚠️ register.queue_state 503")
    lines += [
        "",
        f"Current log:      {log.name if log else 'Not found'}",
        f"Log last updated: {duration(age)} ago",
        f"Disk available:   {free:.1f} GB",
        "",
        f"Refresh every {REFRESH}s | Ctrl+C closes this window only; IOTA keeps running",
        "All Run pools: ~/.iota-guardian/show_runs.sh",
        "==============================================",
    ]
...
def main():
    try:
        ...
        print("\nLocal display closed. IOTA and background monitoring continue running.")
...
try:
    ...
    print("The official Run API is temporarily unavailable: " + str(exc))
    print("This affects Run lookup only. Local monitoring and IOTA training are unaffected.")
    raise SystemExit(1)
...
print("===== Current Macrocosmos IOTA Run Pools =====")
if not runs:
    print("The API responded, but no Run was recognized. The API format may have changed.")
    raise SystemExit(2)
for rid in sorted(runs, key=key, reverse=True):
    ...
    if used is not None and cap is not None:
        detail.append(f"{used}/{cap}, available {max(0, cap-used)}")
    elif used is not None: detail.append(f"current {used}")
    elif cap is not None: detail.append(f"capacity {cap}")
    print(f"{rid}" + (" | " + " | ".join(detail) if detail else ""))
print(f"Total recognized: {len(runs)} Runs")
PY
...
duration(){ local s=${1:-0}; local h=$((s/3600)); local m=$(((s%3600)/60)); ((h>0)) && echo "${h}h ${m}m" || echo "${m}m"; }
send(){
...
(( ${#FILES[@]} > 0 )) || finish NO_LOG down "🔴 No log | NO_LOG | IOTA log not found | Disk ${D}%" 999999
...
(( APP==1 || CLI==1 )) || finish STOPPED down "🔴 IOTA stopped | STOPPED | Open IOTA and click Start training | Log age ${AGE}s | Disk ${D}%" "$AGE"
(( AGE<=180 )) || finish STALE down "🔴 Log stale | STALE | No update for ${AGE}s | Process may be unresponsive | Disk ${D}%" "$AGE"
...
RUNINFO=""; [[ -n "$RUNID" ]] && RUNINFO="Run ${RUNID}"; [[ -n "$LAYER" ]] && RUNINFO="${RUNINFO}${RUNINFO:+ · }L${LAYER}"; [[ -n "$EPOCH" ]] && RUNINFO="${RUNINFO}${RUNINFO:+ · }E${EPOCH}"; [[ -n "$RUNINFO" ]] || RUNINFO="Run pending"
...
ATXT="No recent valid computation"; [[ "$ACTIVEAGE" != 999999 ]] && ATXT="Last valid computation $(duration "$ACTIVEAGE") ago"
WARN=""
[[ -n "${REGERROR:-}" && "${REGERRORAGE:-999999}" -le 21600 ]] && WARN=" | Previous confirmation failed: ${REGERROR}"
[[ "${QUEUEWARNAGE:-999999}" -le 600 ]] && WARN="${WARN} | Scheduler API 503"
NETWARN=""
...
case "$STATE" in
 TRAINING) STATUS=up; MSG="🟢 Active computation | TRAINING | ${RUNINFO} | ${DETAIL} | ${ATXT} | CPU ${CPU}% | P2P ${P2P:-pending}${NETWARN} | Log age ${AGE}s" ;;
 MERGING) STATUS=up; MSG="🟡 Merging phase | MERGING | ${RUNINFO} | No active computation is expected in this phase | ${ATXT} | Log age ${AGE}s" ;;
 SYNCING) STATUS=up; MSG="🟡 Weight synchronization | SYNCING | ${RUNINFO} | Uploading, downloading, or applying model weights${NETWARN} | CPU ${CPU}% | Log age ${AGE}s" ;;
 RUN_IDLE) STATUS=up; MSG="🟠 Waiting inside Run | RUN_IDLE | ${RUNINFO} | Assigned to a Run but no recent valid computation | ${ATXT}${NETWARN} | CPU ${CPU}% | Log age ${AGE}s" ;;
 INITIALIZING) STATUS=up; MSG="🟠 Training pool initialization | INITIALIZING | ${RUNINFO} | No valid computation yet | Log age ${AGE}s" ;;
 CONFIRMED) STATUS=up; MSG="🔵 Queue confirmation received | CONFIRMED | Confirmed by server; Run not assigned yet | Log age ${AGE}s" ;;
 PROCESSING) STATUS=up; MSG="🟠 Registration processing | PROCESSING | Device validation and Run assignment in progress | Log age ${AGE}s" ;;
 REG_FAILED) STATUS=down; MSG="🔴 Registration confirmation failed | REG_FAILED${REGERROR:+ | ${REGERROR}} | The client normally rejoins the queue automatically | Log age ${AGE}s" ;;
 QUEUED) STATUS=up; MSG="🔵 Registration queue | QUEUED${POS:+ | Position ${POS}} | Queued for ${QTXT:-0m} | Not training yet${WARN} | Log age ${AGE}s" ;;
 RESETTING) STATUS=down; MSG="🔴 Registration reset | RESETTING | Run registration lost; waiting to rejoin the queue | Log age ${AGE}s" ;;
 CRASHED) STATUS=down; MSG="🔴 Process crashed | CRASHED | Miner process exited unexpectedly | Log age ${AGE}s" ;;
 STOPPED) STATUS=down; MSG="🔴 IOTA stopped | STOPPED | Click Start training | Log age ${AGE}s" ;;
 *) STATUS=down; MSG="🔴 Status unconfirmed | UNKNOWN | Process exists but no confirmed state was found | CPU ${CPU}% | Log age ${AGE}s" ;;
esac
((D<90)) || { STATUS=down; MSG="🔴 Critically low disk space | ${D}% | $MSG"; }
...
echo "$NOW | IOTA logs ${B}KB→${A}KB | Freed ${F}KB" >> "$BASE/cleanup.log"
...
cat > "$R" <<EOF2
IOTA Guardian V6.3 Daily Report
Date: $TODAY
Machine: $(cat "$BASE/node_name" 2>/dev/null)
Active computation: approximately $(count TRAINING) minutes
Merging phase: approximately $(count MERGING) minutes
Weight synchronization: approximately $(count SYNCING) minutes
Waiting inside Run: approximately $(count RUN_IDLE) minutes
Queued: approximately $(count QUEUED) minutes
Registration: CONFIRMED $(count CONFIRMED) / PROCESSING $(count PROCESSING) / REG_FAILED $(count REG_FAILED) minutes
Failures: RESETTING $(count RESETTING) / CRASHED $(count CRASHED) / STOPPED $(count STOPPED) / STALE $(count STALE) minutes
Current: $(cat "$BASE/current_message" 2>/dev/null)
EOF2
...
echo "Machine: $(cat "$BASE/node_name" 2>/dev/null)"
echo "Current: $(cat "$BASE/current_message" 2>/dev/null)"
echo "Recent state changes:"; tail -n 15 "$BASE/events.csv" 2>/dev/null
...
# Disable only earlier versions of this monitor. Do not touch IOTA, wallets, hotkeys, or third-party hub agents.
for plist in "$LAUNCH"/com.baibai.iota-log-monitor.plist "$LAUNCH"/com.baibai.iota-guardian*.plist; do
  ...
  [[ -n "$old_label" && "$old_label" == "${${plist:t}%.plist}" ]] || { echo "The previous monitor label does not match. Installation stopped: $plist"; exit 1; }
...
echo
echo "✅ IOTA Guardian V6.3 installation/upgrade completed"
echo "Machine: $(cat "$BASE/node_name")"
echo "Checks every 60 seconds; a log is marked STALE only after 180 seconds without an update."
echo "TRAINING=active computation; MERGING=merging; SYNCING=weight synchronization; CONFIRMED/PROCESSING=registration; QUEUED=waiting in queue."
echo "View all Run pools: $BASE/show_runs.sh"
echo "Open the local live dashboard: $BASE/show_monitor.sh"
echo "This tool does not start, stop, restart, or click IOTA, and it does not modify wallets or miners."
echo "Wait about 60 seconds, then check Uptime Kuma."
